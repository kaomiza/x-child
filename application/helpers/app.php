    ...
    $(function() {
      ...
      var Router = Backbone.Router.extend({
        ...
        join: function() {
          ...
          this.container.removeClass("page-dashboard").addClass("page-join");
      ...
      configTxt +=
        " endpoints: " +
        (CONFIG.APP_CONFIG.endpoints ?
          JSON.stringify(CONFIG.APP_CONFIG.endpoints) :
          "test server");
      ...
      $(window).scroll(function() {
        if (
          ...
          app.helpers.renderUsers.condition.page != undefined &&
          !app.helpers.renderUsers.stop
        ) {
          app.helpers.renderUsers.condition.page += 1;
          ui.insertOccupants(false).then(
            function(users) {
              var ids = users.map(function(user) {
                return user.id;
              });
              app.users = users.concat(
                app.users.filter(function(item) {
      ...
      $(document).on("submit", ".j-join", function() {
        var $form = $(this);
        var data = {
          username: "<?php echo $this->session->userdata('U_fname') . ' ' . $this->session->userdata('U_lname'); ?>",
          login: "<?php echo $this->session->userdata('U_user'); ?>",
        };
      ...
      $(document).on("click", ".j-actions", function() {
        var $btn = $(this),
          ...
          mediaParams = {
            audio: {
              deviceId: $audioSourceFilter.val() ?
                $audioSourceFilter.val() : undefined,
            },
            video: {
              deviceId: $videoSourceFilter.val() ? {
                exact: $videoSourceFilter.val()
              } : undefined,
            },
            options: {
              muted: true,
        ...
        if ($btn.hasClass("hangup")) {
          ...
          if (isAudio) {
            ...
            document
              .querySelector('.j-actions[data-call="video"]')
              .setAttribute("hidden", true);
          } else {
            document
              .querySelector('.j-actions[data-call="audio"]')
              .setAttribute("hidden", true);
          }
      ...
      $(document).on("click", ".j-callees__callee__video", function() {
        ...
        if (
          app.currentSession.peerConnections[userId].stream &&
          !$that.srcObject
        ) {
          if ($that.hasClass("active")) {
            ...
            activeClass = _.intersection(
              $that.attr("class").split(/\s+/),
              app.filter.names.split(/\s+/)
            );
            ...
            app.currentSession.attachMediaStream(
              "main_video",
              app.currentSession.peerConnections[userId].stream
            );
      ...
      $(document).on(
        "click",
        '.j-caller__ctrl[data-target="screen"]',
        function() {
          if (_.isEmpty(app.currentSession)) {
            return false;
          }
      ...
      QB.webrtc.onCallStatsReport = function onCallStatsReport(
        ...
        if (stats.remote.video.bitrate) {
          $("#bitrate_" + userId).text(
            "video bitrate: " + stats.remote.video.bitrate
          );
        } else if (stats.remote.audio.bitrate) {
          $("#bitrate_" + userId).text(
            "audio bitrate: " + stats.remote.audio.bitrate
          );
        }
      };
      ...
      QB.webrtc.onCallListener = function onCallListener(session, extension) {
        ...
        QB.users.get(Number(session.initiatorID), function(error, user) {
          if (error) {
            ...
            if (
              app.currentSession.state !== QB.webrtc.SessionConnectionState.CLOSED
            ) {
      ...
      QB.webrtc.onStopCallListener = function onStopCallListener(
        ...
        app.helpers.notifyIfUserLeaveCall(
          session,
          userId,
          "hung up the call",
          "Hung Up"
      ...
      QB.webrtc.onSessionConnectionStateChangedListener = function onSessionConnectionStateChangedListener(
        ...
        console.log(
          "Сonnection state:",
          connectionState,
          statesPeerConn[connectionState]
        );
        console.groupEnd();

        var connectionStateName = _.invert(QB.webrtc.SessionConnectionState)[
            connectionState
          ],
        ...
        if (connectionState === QB.webrtc.SessionConnectionState.CLOSED) {
          ...
          if (app.mainVideo === userId) {
            ...
            for (var key in app.currentSession.peerConnections) {
              if (
                ...
                app.currentSession.attachMediaStream(
                  "main_video",
                  app.currentSession.peerConnections[key].stream
                );
                app.mainVideo = key;
                break;
              }
          ...
          if (!_.isEmpty(app.currentSession)) {
            if (
              Object.keys(app.currentSession.peerConnections).length === 1 ||
              userId === app.currentSession.initiatorID
            ) {
              $(ui.income_call).modal("hide");
              document.getElementById(sounds.rington).pause();
            }
          ...
          if (
            app.currentSession.currentUserID === app.currentSession.initiatorID &&
            !isCallEnded
          ) {
            var userInfo = _.findWhere(app.users, {
              id: +userId
          ...
          QB.webrtc
            ...
            .then(function(devices) {
              var isVideoInput = kind === "videoinput",
                $select = isVideoInput ?
                $(ui.videoSourceFilter) :
                $(ui.audioSourceFilter);
      ...
      function switchMediaTracks() {
        ...
        if (
          (typeof localVideo == "object" && !localVideo.srcObject) ||
          !app.currentSession
        ) {
          return true;
        }

        var audioDeviceId = $(ui.audioSourceFilter).val() ?
          $(ui.audioSourceFilter).val() :
          undefined,
          videoDeviceId = $(ui.videoSourceFilter).val() ? {
            exact: $(ui.videoSourceFilter).val()
          } :
          undefined,
          deviceIds = {
            audio: audioDeviceId,
